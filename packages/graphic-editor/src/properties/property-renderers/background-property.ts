...
type BackgroundProp = Extract<GraphicBlockProperty, { kind: 'background' }>;
...
export function createBackgroundRenderer(
  ...
  const currentColor = (readValue(rendCtx.node, property.colorPath) as string | undefined) ?? '#ffffff';
  ...
  swatch.addEventListener('click', () => {
    ...
    colorPickerInstance.show({
      ...
      initialColor: (readValue(rendCtx.node, property.colorPath) as string) ?? '#ffffff',
      labels: {
  ...
  return {
    ...
    setValue(updatedNode: GraphicElement) {
      ...
      const newColor = (readValue(updatedNode, property.colorPath) as string | undefined) ?? '#ffffff';
